...
def redraw_window(win, stuff_to_draw):
    ...
    for bullet in bullets:
        win.blit(bullet_image, (bullet[0]+100, bullet[1]+68))
        bullet[0] += 25
        if bullet[0] >= dimensions.WIDTH:
...
def main():
    ...
    # zombie stuff
    bullet_last_tick = pygame.time.get_ticks()
    ...
    while is_running:
        ...
        did_shoot = player1.shoot()

        if did_shoot != None :
            bullet_current_tick = pygame.time.get_ticks()

            if bullet_current_tick -bullet_last_tick >= bullet_interval:
                bullet_current_tick

        #if did_shoot is not None:
        #    bullet_current_tick = pygame.time.get_ticks()  # getting the current time ticks

        #    if bullet_current_tick - bullet_last_tick >= bullet_interval:
        #        bullet_current_tick = bullet_last_tick
        #        x, y = network.player_did_shoot(did_shoot)
        #        bullets.append([x, y])
